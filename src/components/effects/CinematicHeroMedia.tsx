...
/** Desktop-only cinematic hero — video, parallax, overlays */
export function CinematicHeroMedia() {
  ...
  useEffect(() => {
    ...
    const onScroll = () => {
      ...
      requestAnimationFrame(() => {
        ...
        if (overlay) {
          overlay.style.opacity = String(0.55 + progress * 0.45);
        }
        container.style.opacity = String(Math.max(1 - progress * 1.4, 0));
        ticking = false;
      });
    };
  ...
  return (
    ...
    >
      ...
      >
        <div className="hero-media-parallax-inner hero-media-layer absolute inset-[-6%]">
          <Image
            src={heroPoster}
            alt=""
            fill
